...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Fetching drivers for {year} round {round_val}...")
        )
        ...
        try:
            ...
            # FastF1 CountryCode is currently returning empty for 2024/2025.
            # Using a fallback dictionary for current known drivers.
            nationality_map = {
                "VER": "NED",
                "PER": "MEX",
                "HAM": "GBR",
            ...
            for drv_num in session.drivers:
                ...
                # Try getting from FastF1, fallback to our map
                nationality = info.get("CountryCode")
                if not nationality:
                    nationality = nationality_map.get(acronym, "Unknown")

                driver, created = Driver.objects.update_or_create(
                    acronym=acronym,
                    defaults={"name": name, "team": team, "nationality": nationality},
                )
                if created:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Successfully synced {drivers_synced} drivers.")
            )
